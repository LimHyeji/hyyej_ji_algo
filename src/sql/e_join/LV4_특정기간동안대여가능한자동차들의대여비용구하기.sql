SELECT C.CAR_ID AS CAR_ID, C.CAR_TYPE AS CAR_TYPE, ROUND(C.DAILY_FEE*((100-P.DISCOUNT_RATE)/100)*30,0) AS FEE
...
WHERE C.CAR_ID IN (SELECT CAR_ID FROM CAR_RENTAL_COMPANY_CAR WHERE CAR_TYPE IN ('세단','SUV'))
